...
// Placeholder tests for Orient reference validation
// These will be implemented once we have the wigner function working
/*
TEST_CASE("Multipole rotation - Orient reference validation",
"[mults][rotation][orient][.slow]") {

    SECTION("Compare with Orient water dimer calculations") {
        // TODO: Load exact multipoles from orient test files
        // TODO: Apply same rotations as orient test cases
        // TODO: Compare results numerically
    }

    SECTION("Validate against Orient S-function calculations") {
        // TODO: Test that rotated multipoles give same interaction energies
        // TODO: Cross-validate with orient interaction calculations
    }
}
*/
...
TEST_CASE("CoordinateSystem - basic functionality", "[mults][coordinates]") {
    ...
    SECTION("Orient coordinates - molecule to grid point") {
        // Orient calculation: molecule at (0,0,0) to grid point at (2,2,2)
        Vec3 ra(0.0, 0.0, 0.0);  // Molecule position
        Vec3 rb(2.0, 2.0, 2.0);  // Grid point
        auto cs = CoordinateSystem::from_points(ra, rb);
        
        // Distance should be sqrt(4 + 4 + 4) = sqrt(12) = 2*sqrt(3)
        double expected_r = 2.0 * std::sqrt(3.0);
        ...
        // Orient coordinates should match: rax = er, rbx = -er
        // For vector from (0,0,0) to (2,2,2): er = (+0.577, +0.577, +0.577)
        // So rax = +0.577, rbx = -0.577
        ...
        // Coordinate validation against Orient values (from (0,0,0) to (2,2,2))
        // Distance = 2*sqrt(3) = 3.46410162, unit components = ±1/sqrt(3) = ±0.57735027
        REQUIRE(cs.r == Approx(2.0 * std::sqrt(3.0)));
        REQUIRE(cs.rax() == Approx(1.0/std::sqrt(3.0)));
        REQUIRE(cs.ray() == Approx(1.0/std::sqrt(3.0)));
        REQUIRE(cs.raz() == Approx(1.0/std::sqrt(3.0)));
        REQUIRE(cs.rbx() == Approx(-1.0/std::sqrt(3.0)));
        REQUIRE(cs.rby() == Approx(-1.0/std::sqrt(3.0)));
        REQUIRE(cs.rbz() == Approx(-1.0/std::sqrt(3.0)));
    }
}
