...
TEST_CASE("Wulff", "[geometry]") {
    ...
    pts << 0.27324055, 0.74569843, 0.24143472, 0.81361223, 0.9912406,
        0.42422966, 0.97878276, 0.47166676, 0.00768781, 0.05601098, 0.45920777,
        0.92948902, 0.04862679, 0.48147977, 0.6230096, 0.90899989, 0.29847469,
        0.6059014;
...
createSphere(T radius, size_t M,
    ...
    for (size_t i = 0; i <= M; i++) {
        ...
        FloatType K =
            FloatType(1) - std::abs(static_cast<FloatType>(
                               static_cast<FloatType>(i) - M / 2.0f)) /
                               static_cast<FloatType>(M / 2.0f);
        const size_t pcount = static_cast<size_t>(1 + K * M + FloatType(1) / 2);
        for (size_t j = 0; j < pcount; j++) {
            FloatType x =
                pcount > 1
                    ? r * std::cos(static_cast<FloatType>(j) / pcount * pi * 2)
                    : 0;
            FloatType z =
                pcount > 1
                    ? r * std::sin(static_cast<FloatType>(j) / pcount * pi * 2)
                    : 0;
...
TEST_CASE("Basic hull", "[qh]") {
    ...
    for (int i = 0; i < 8; i++) {
        pc.col(i) = Eigen::Matrix<FloatType, 3, 1>(
...
TEST_CASE("Planar hull", "[qh]") {
    ...
    pc.col(1) =
        Eigen::Matrix<FloatType, 3, 1>(-3.000000f, 0.250000f, -0.800000f);
    pc.col(2) =
        Eigen::Matrix<FloatType, 3, 1>(-3.125000f, 0.250000f, -0.750000);
    pc.col(3) =
        Eigen::Matrix<FloatType, 3, 1>(-3.125000f, -0.250000f, -0.750000);
...
TEST_CASE("Circle cylinder hull", "[qh]") {
    ...
    REQUIRE(hull.indices().size() / 3 ==
            static_cast<size_t>(pc2.cols()) * 2 - 4);
}
...
TEST_CASE("Half edge output", "[qh]") {
    ...
    for (int h = 1000; h < 1008; h++) {
        pc.col(h) = Eigen::Matrix<FloatType, 3, 1>(
            h & 1 ? -2 : 2, h & 2 ? -2 : 2, h & 4 ? -2 : 2);
    }
...
TEST_CASE("Sphere tests", "[qh]") {
    ...
    FloatType y = 1;
    for (;;) {
        auto pc = createSphere<FloatType>(
            1, 100, Eigen::Matrix<FloatType, 3, 1>(0, y, 0));
        auto hull = qh.getConvexHull(pc, true);
        y *= 15;
        y /= 10;
        if (hull.indices().size() == 12) {
    ...
    for (;;) {
        auto pc = createSphere<FloatType>(
            1, i, Eigen::Matrix<FloatType, 3, 1>(0, 0, 0));
