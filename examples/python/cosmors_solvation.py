...
and then contracts the solute's screening-charge segments against a cached
segment ensemble for the solvent.

Run:
    python examples/python/cosmors_solvation.py [water.xyz] [solvent]
"""
...
def main():
    ...
    available = occpy.available_cosmo_rs_solvents()
    if solvent not in available:
        print(f"no segment ensemble for '{solvent}'; have: {', '.join(available)}")
        sys.exit(1)
    ...
    result = occpy.cosmo_rs_solvation_free_energy(mol, solvent, settings)
    e = result.energy

    print(f"\ncavity: {result.cavity_area:.2f} A^2, {result.cavity_volume:.2f} A^3")
    print(f"rings:  {result.num_rings}")
    print(f"\nSolvation free energy in {solvent} (kJ/mol):")
