...
template<>
inline int shell_index_cartesian<ShellOrder::Default>(int i, int j, int k, int l) {
    return ((((l - i + 1) * (l - i)) >> 1) + l - i - j);
}

template<>
inline int shell_index_cartesian<ShellOrder::Gaussian>(int i, int j, int k, int l) {
    int idx_found = -1;
    int idx = 0;
...
template<>
inline int shell_index_spherical<ShellOrder::Molden>(int l, int m) {
    return 2 * std::abs(m) + (m > 0 ? -1 : 0);
}
