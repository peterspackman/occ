...
template <typename T> class MeshBuilder {
  public:
    ...
    struct Face {
        ...
        std::uint8_t
            m_horizonEdgesOnCurrentIteration : 3; // Bit for each half edge
                                                  // assigned to this face, each
                                                  // being 0 or 1 depending on
                                                  // whether the edge belongs to
                                                  // horizon edge
        std::unique_ptr<std::vector<size_t>> m_pointsOnPositiveSide;

        Face()
            : m_he(std::numeric_limits<size_t>::max()),
              m_mostDistantPointDist(0), m_mostDistantPoint(0),
    ...
    // When the mesh is modified and faces and half edges are removed from it,
    // we do not actually remove them from the container vectors. Insted, they
    // are marked as disabled which means that the indices can be reused when we
    // need to add new faces and half edges to the mesh. We store the free
    // indices in the following vectors.
    ...
    // Mark a face as disabled and return a pointer to the points that were on
    // the positive of it.
