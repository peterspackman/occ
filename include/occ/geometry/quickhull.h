...
#include <deque>
#include <limits>
#include <occ/geometry/convex_hull.h>
#include <occ/geometry/half_edge_mesh.h>
#include <occ/geometry/math_utils.h>
#include <occ/geometry/mesh.h>
#include <occ/geometry/plane.h>
#include <occ/geometry/pool.h>
#include <vector>
 ...
 *
 * Modified by Peter Spackman @ https://github.com/peterspackman/quickhull
 *
 *
...
struct DiagnosticsData {
    size_t m_failedHorizonEdges; // How many times QuickHull failed to solve the
                                 // horizon edge. Failures lead to degenerated
                                 // convex hulls.

    DiagnosticsData() : m_failedHorizonEdges(0) {}
};

template <typename FloatType> FloatType defaultEps();

template <typename FloatType> class QuickHull {
    using vec3 = Eigen::Matrix<FloatType, 3, 1>;

    FloatType m_epsilon, m_epsilonSquared, m_scale;
    bool m_planar;
    std::vector<vec3> m_planarPointCloudTemp;
    Eigen::Matrix<FloatType, 3, Eigen::Dynamic> m_vertexData;
    ...
    struct FaceData {
        size_t m_faceIndex;
        size_t m_enteredFromHalfEdge; // If the face turns out not to be
                                      // visible, this half edge will be marked
                                      // as horizon edge
    ...
    // QuickHull iteration proceeds. m_extremeValues must be properly set up
    // when this is called.
    void setupInitialTetrahedron();

    // Given a list of half edges, try to rearrange them so that they form a
    // loop. Return true on success.
    ...
    // especially at the the end of the iteration. When a face is removed from
    // the mesh, its associated point vector, if such exists, is moved to the
    // index vector pool, and when we need to add new faces with points on the
    // positive side to the mesh, we reuse these vectors. This reduces the
    // amount of std::vectors we have to deal with, and impact on performance is
    // remarkable.
    Pool<std::vector<size_t>> m_indexVectorPool;
    inline std::unique_ptr<std::vector<size_t>> getIndexVectorFromPool();
    inline void
    reclaimToIndexVectorPool(std::unique_ptr<std::vector<size_t>> &ptr);

    // Associates a point with a face if the point resides on the positive side
    // of the plane. Returns true if the points was on the positive side.
    inline bool addPointToFace(typename MeshBuilder<FloatType>::Face &f,
                               size_t pointIndex);

    // This will update m_mesh from which we create the ConvexHull object that
    ...
    void
    buildMesh(const Eigen::Matrix<FloatType, 3, Eigen::Dynamic> &pointCloud,
              bool CCW, FloatType eps);
  ...
  public:
    ...
    //   useOriginalIndices: should the output mesh use same vertex indices as
    //   the original point cloud. If this is false,
    //      then we generate a new vertex buffer which contains only the
    ...
    getConvexHull(const Eigen::Matrix<FloatType, 3, Eigen::Dynamic> &pointCloud,
                  bool CCW, FloatType eps = defaultEps<FloatType>());
    ...
    //   the output mesh triangles should have CCW orientation
    //   useOriginalIndices: should the output mesh use same vertex indices as
    //   the original point cloud. If this is false,
    //      then we generate a new vertex buffer which contains only the
    //      vertices that are part of the convex hull.
    ...
    //   side of it (for a point cloud with scale 1)
    ConvexHull<FloatType>
    getConvexHull(const FloatType *vertexData, size_t vertexCount, bool CCW,
    ...
    //   distance to a plane to consider a point being on positive side of it
    ...
    getConvexHullAsMesh(const FloatType *vertexData, size_t vertexCount,
                        bool CCW, FloatType eps = defaultEps<FloatType>());
...
void QuickHull<T>::reclaimToIndexVectorPool(
    ...
    if ((oldSize + 1) * 128 < ptr->capacity()) {
        ...
        // iteration when faces have many points on their positive side. Later
        // on, smaller vectors will suffice.
        ptr.reset(nullptr);
        return;
    }
...
bool QuickHull<T>::addPointToFace(typename MeshBuilder<T>::Face &f,
                                  size_t pointIndex) {
    const T D = mathutils::getSignedDistanceToPlane(
        m_vertexData.col(pointIndex), f.m_P);
