    ...
    auto compute = [&](int thread_id) {
        ...
        for (auto s1 = 0l, s12 = 0l; s1 != nshells; ++s1) {
            ...
            for (auto s2 : shellpair_list.at(s1)) {
                ...
                // "copy" lambda copies shell set \c idx to the operator matrix
                // with index \c op
                ...
                case 0:
                    ...
                    // map deriv quanta for this shell pair to the overall deriv
                    // quanta
                    //
                    // easiest to explain with example:
                    // in sto-3g water shells 0 1 2 sit on atom 0, shells 3 and
                    // 4 on atoms 1 and 2 respectively each call to
                    // engine::compute for nuclear ints will return derivatives
                    // with respect to 15 coordinates, obtained as 3 (x,y,z)
                    // times 2 + 3 = 5 centers (2 centers on which shells sit +
                    // 3 nuclear charges) (for overlap, kinetic, and emultipole
                    // ints we there are only 6 coordinates
                    //  since the operator is coordinate-independent, or
                    //  derivatives with respect to the operator coordinates are
                    //  not computed)
                    //
                ...
                case 2: {
                    ...
                    // higher-order derivs e.g. d2 (s1|s2) / dX dY involves
                    // several cases:
                    // 1. only s1 (or only s2) depends on X AND Y (i.e. X and Y
                    // refer to same atom) =>
                    //    d2 (s1|s2) / dX dY = (d2 s1 / dX dY | s2)
                    // 2. s1 depends on X only, s2 depends on Y only (or vice
                    // versa) =>
                    //    d2 (s1|s2) / dX dY = (d s1 / dX | d s2 / dY)
                    // 3. s1 AND s2 depend on X AND Y (i.e. X and Y refer to
                    // same atom) =>
                    //    case A: X != Y
                    //    d2 (s1|s2) / dX dY = (d2 s1 / dX dY | s2) + (d s1 / dX
                    //    | d s2 / dY)
                    //      + (d s1 / dY | d s2 / dX) + (s1| d2 s2 / dX dY )
                    //    case B: X == Y
                    //    d2 (s1|s2) / dX2 = (d2 s1 / dX2 | s2) + 2 (d s1 / dX |
                    //    d s2 / dX)
                    //      + (s1| d2 s2 / dX2 )
    ...
    (std::min((i), (j))) * ((n2) - (std::min((i), (j))) - 1) / 2 +             \
        (std::max((i), (j)))
                    ...
                    for (auto c1 = 0; c1 != nderivcenters_shset; ++c1) {
                        auto a1 =
                            (c1 == 0) ? atom1 : ((c1 == 1) ? atom2 : c1 - 2);
                        auto coord1 = 3 * a1;
                        for (auto xyz1 = 0; xyz1 != 3; ++xyz1, ++coord1) {

                            for (auto c2 = c1; c2 != nderivcenters_shset;
                                 ++c2) {
                                auto a2 = (c2 == 0)
                                              ? atom1
                                              : ((c2 == 1) ? atom2 : c2 - 2);
                                ...
                                for (auto xyz2 = xyz2_start; xyz2 != 3;
                                    ...
                                    double scale =
                                        (coord1 == coord2 && c1 != c2) ? 2.0
                                                                       : 1.0;

                                    const auto coord12 = upper_triangle_index(
                                        two_times_ncoords, coord1, coord2);
        ...
        for (auto s1 = 0l, s12 = 0l; s1 != nsh1; ++s1) {
            ...
            for (auto s2 = 0; s2 <= s2_max; ++s2, ++s12) {
                ...
                engines[thread_id].compute2<Kernel, BraKet::xx_xx, 0>(
                    bs1[s1], bs2[s2], bs1[s1], bs2[s2]);
                assert(buf[0] != nullptr &&
                       "to compute Schwarz ints turn off primitive screening");

                // to apply Schwarz inequality to individual integrals must use
                // the diagonal elements to apply it to sets of functions (e.g.
                // shells) use the whole shell-set of ints here
...
compute_2body_fock_deriv(const BasisSet &obs,
    ...
    Mat D_shblk_norm = compute_shellblock_norm(
        obs, D); // matrix of infty-norms of shell blocks
            ...
            for (const auto &s2 : shellpair_list.at(s1)) {
                ...
                for (auto s3 = 0; s3 <= s1; ++s3) {
                    ...
                    for (const auto &s4 : shellpair_list.at(s3)) {
                        ...
                        // computes contribution from shell set \c idx to the
                        // operator matrix with index \c op
                        auto add_shellset_to_dest = [&](std::size_t op,
                                                        std::size_t idx,
                                ...
                                for (auto f2 = 0; f2 != n2; ++f2) {
                                    ...
                                    for (auto f3 = 0; f3 != n3; ++f3) {
                                        ...
                                        for (auto f4 = 0; f4 != n4;
                                            ...
                                            g(bf1, bf3) -=
                                                0.25 * D(bf2, bf4) * wvalue;
                                            g(bf2, bf4) -=
                                                0.25 * D(bf1, bf3) * wvalue;
                                            g(bf1, bf4) -=
                                                0.25 * D(bf2, bf3) * wvalue;
                                            g(bf2, bf3) -=
                                                0.25 * D(bf1, bf4) * wvalue;
                                        }
                                    }
                                }
                            }
                        ...
                        engine.compute2<Operator::coulomb, BraKet::xx_xx,
                                        deriv_order>(obs[s1], obs[s2], obs[s3],
                                                     obs[s4]);
                        if (buf[0] == nullptr)
                        ...
                        case 1: {
                            for (auto d = 0; d != 12; ++d) {
                                ...
                                add_shellset_to_dest(thread_id * nderiv + coord,
                                                     d, coord1, coord2);
...
#define upper_triangle_index(n2, i, j)                                         \
    (std::min((i), (j))) * ((n2) - (std::min((i), (j))) - 1) / 2 +             \
        (std::max((i), (j)))
                            // look over shellsets in the order in which they
                            // appear
                            std::size_t shellset_idx = 0;
                            for (auto c1 = 0; c1 != 4; ++c1) {
                                auto a1 = shell_atoms[c1];
                                auto coord1 = 3 * a1;
                                for (auto xyz1 = 0; xyz1 != 3;
                                     ++xyz1, ++coord1) {
                                        ...
                                        for (auto xyz2 = xyz2_start; xyz2 != 3;
                                            ...
                                            const auto coord12 =
                                                upper_triangle_index(
                                                    ncoords_times_two, coord1,
                                                    coord2);
                                            auto op =
                                                thread_id * nderiv + coord12;
                                            add_shellset_to_dest(
                                                op, shellset_idx, coord1,
                                                coord2, scale);
                                            ++shellset_idx;
                                        }
                                    }
...
#undef upper_triangle_index
                        ...
                        default:
                            assert(deriv_order <= 2 &&
                                   "support for 3rd and higher derivatives of "
                                   "the Fock "
