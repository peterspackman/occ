...
public:
  PointChargeCorrectedProcedure(Proc &proc,
    ...
    m_nuclear_external_energy =
        m_proc.nuclear_point_charge_interaction_energy(m_point_charges);
    occ::log::info("Point charge correction initialized with {} charges",
                   m_point_charges.size());
    occ::log::info("Nuclear-point charge interaction energy: {:.8f}",
                   m_nuclear_external_energy);
  }
  ...
  auto
  compute_point_charge_interaction_matrix(const PointChargeList &pc) const {
  ...
  std::string name() const {
    return fmt::format("{}+PointCharges({})", m_proc.name(),
                       m_point_charges.size());
  }
...
public:
  WolfSumCorrectedProcedure(Proc &proc, const PointChargeList &point_charges,
      ...
      : m_proc(proc), m_point_charges(point_charges),
        m_molecular_charges(molecular_charges), m_alpha(alpha),
        m_cutoff(cutoff) {
    ...
    occ::log::info("Wolf sum correction initialized with {} charges",
                   m_point_charges.size());
    occ::log::info("Wolf parameters: alpha={:.4f}, cutoff={:.2f}", m_alpha,
                   m_cutoff);
    occ::log::info("Nuclear-Wolf interaction energy: {:.8f}",
                   m_nuclear_external_energy);
  }
  ...
  void update_core_hamiltonian(const MolecularOrbitals &mo, Mat &H) {
    ...
    Mat V_wolf = m_proc.compute_wolf_interaction_matrix(
        m_point_charges, m_molecular_charges, m_alpha, m_cutoff);
  ...
  std::string name() const {
    return fmt::format("{}+Wolf(α={:.3f},rc={:.1f})", m_proc.name(), m_alpha,
                       m_cutoff);
  }
