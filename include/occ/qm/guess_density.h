...
void update_occupation_subshell(output_iterator &destination, size_t size,
                                size_t &num_electrons_remaining) {
    ...
    const double electrons_per_orbital =
        (electrons_allocated % size == 0)
            ...
            : (static_cast<double>(electrons_allocated)) / size;
    for (size_t f = 0; f != size; ++f, destination++)
