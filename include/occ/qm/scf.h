...
template <typename Procedure> struct SCF {
  ...
  void set_conditioning_orthogonalizer() {
    ...
    if (mo.kind == Unrestricted) {
      ...
      g = core::conditioning_orthogonalizer(S, S_condition_number_threshold);
    }
  ...
  void set_core_matrices() {
    ...
    case SpinorbitalKind::Unrestricted: {
      ...
      block::b(V) = block::a(V);
      if (calc_ecp) {
        block::a(Vecp) = m_procedure.compute_effective_core_potential_matrix();
        block::b(Vecp) = block::a(Vecp);
      }
      if (calc_pc) {
        block::a(Vpc) = m_procedure.compute_point_charge_interaction_matrix(
            m_point_charges);
    ...
    case SpinorbitalKind::General: {
      ...
      if (calc_ecp) {
        block::aa(Vecp) = m_procedure.compute_effective_core_potential_matrix();
        block::bb(Vecp) = block::aa(Vecp);
      }
      if (calc_pc) {
        block::aa(Vpc) = m_procedure.compute_point_charge_interaction_matrix(
            m_point_charges);
  ...
  void compute_initial_guess() {
    ...
    if (m_procedure.aobasis().name() == OCC_MINIMAL_BASIS) {
      D_minbs = compute_soad(
          m_procedure
              .compute_overlap_matrix()); // compute guess in minimal basis
      switch (mo.kind) {
      case Restricted:
        mo.D = D_minbs;
        break;
  ...
  double compute_scf_energy() {
    ...
    do {
      ...
      update_scf_energy(incremental);
      ediff_rel =
          std::abs((energy["electronic"] - ehf_last) / energy["electronic"]);
