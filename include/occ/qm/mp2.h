...
public:
  ...
  MP2(const AOBasis &basis, const AOBasis &aux_basis,
  ...
  void
  set_frozen_core_auto(); // Automatically determine frozen core based on atoms
  void set_virtual_cutoff_energy(double cutoff_hartree) {
    m_virtual_cutoff = cutoff_hartree;
  }
  ...
  struct Results {
    ...
    std::map<std::pair<size_t, size_t>, double>
        pair_energies; // MP2 pair energies
...
private:
  ...
  void log_virtual_truncation_info(size_t n_occ_total, size_t n_virt_total,
                                   ...
                                   const Vec &orbital_energies) const;
  void estimate_memory_requirements(size_t n_ao, size_t n_occ_active,
