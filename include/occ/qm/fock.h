...
{
public:
    FockBuilder(size_t max_nprim, size_t max_l, double precision = std::numeric_limits<double>::epsilon(), int derivative_order=0)
    {
        m_coulomb_engines.reserve(occ::parallel::nthreads);
        m_coulomb_engines.emplace_back(Operator::coulomb, max_nprim, max_l, derivative_order);
        m_coulomb_engines[0].set_precision(precision); // shellset-dependent precision
        ...
        const Mat &Schwarz = Mat()) const
    {
        ...
        auto lambda = [&](int thread_id) {
            ...
            for (size_t s1 = 0, s1234 = 0; s1 != nshells; ++s1) {
                ...
                auto n1 = shell1.size();      // number of basis functions in this shell
        ...
        occ::timing::start(occ::timing::category::lambda);
        occ::parallel::parallel_do(lambda);
        occ::timing::stop(occ::timing::category::lambda);
...
namespace impl {
static std::vector<libint2::Engine> coulomb_engines;
}
    ...
    const Mat &Schwarz = Mat())
{
    ...
    // construct the 2-electron repulsion integrals engine pool
    auto& engines = impl::coulomb_engines;
    if(engines.size() < nthreads)
    {
        engines.resize(nthreads);
        engines[0] = libint2::Engine(Operator::coulomb, obs.max_nprim(), obs.max_l(), 0);
