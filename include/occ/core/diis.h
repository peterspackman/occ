...
public:
  ...
  void extrapolate(T& x, T& error, bool extrapolate_error = false)
  {
    ...
    if (m_errors.size() == m_diis_subspace_size) {
      ...
      Mat Bcrop = m_B.bottomRightCorner(m_diis_subspace_size-1,m_diis_subspace_size-1);
    ...
    for (size_t i = 0; i < nvec - 1; i++) {
      m_B(i, nvec - 1) = m_B(nvec - 1, i) = m_errors[i].cwiseProduct(m_errors[nvec - 1]).sum();
    }
    m_B(nvec - 1, nvec - 1) = m_errors[nvec - 1].cwiseProduct(m_errors[nvec - 1]).sum();
        ...
        for (size_t k = nskip, kk = 1; k < nvec; ++k, ++kk) {
          if (!do_mixing || m_extrapolated.empty()) {
            ...
            if (extrapolate_error)
              ...
              x.noalias() += c[kk] * m_mixing_fraction * m_extrapolated[k];
          }
        }
      }
    } // do DIIS
