  ...
  public:
    using GraphContainer =
        boost::adjacency_list<boost::vecS, boost::vecS, boost::directedS,
                              Vertex, Edge>;
    using vertex_t =
        typename boost::graph_traits<GraphContainer>::vertex_descriptor;
    using edge_t =
        typename boost::graph_traits<GraphContainer>::edge_descriptor;
    using edge_pair = std::pair<edge_t, edge_t>;
    using vertex_iter =
        typename boost::graph_traits<GraphContainer>::vertex_iterator;
    using edge_iter =
        typename boost::graph_traits<GraphContainer>::edge_iterator;
    using adjacency_iter =
        typename boost::graph_traits<GraphContainer>::adjacency_iterator;
    using degree_t =
        typename boost::graph_traits<GraphContainer>::degree_size_type;
    ...
    using edge_property_t =
        typename boost::edge_bundle_type<GraphContainer>::type;
    using vertex_property_t =
        typename boost::vertex_bundle_type<GraphContainer>::type;
    BondGraph() {}
    ...
    std::optional<edge_t> add_edge(vertex_t i, vertex_t j,
                                   const edge_property_t &edge) {
        bool b;
        edge_t e;
        std::tie(e, b) = boost::add_edge(i, j, edge, m_graph);
        if (b)
            return e;
        else {
            return std::nullopt;
    ...
    std::vector<Vertex> neighbor_list(vertex_t i) const {
        std::vector<Vertex> n;
        typename boost::graph_traits<GraphContainer>::adjacency_iterator vi,
            vi_end;
        for (std::tie(vi, vi_end) = boost::adjacent_vertices(i, m_graph);
             vi != vi_end; ++vi) {
