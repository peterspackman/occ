...
public:
  ...
  PromoleculeDensity(const occ::core::Molecule &mol,
  ...
  OCC_ALWAYS_INLINE std::pair<float, FVec3>
  density_and_gradient(const FVec3 &pos) const {
    double result{0.0};
    FVec3 grad = FVec3::Zero();
    for (const auto &[interp, interp_positions, threshold] :
         m_atom_interpolators) {
...
private:
  void initialize_interpolators(Eigen::Ref<const IVec> numbers,
