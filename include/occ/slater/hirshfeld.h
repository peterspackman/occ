...
public:
    StockholderWeight(const PromoleculeDensity &inside, const PromoleculeDensity &outside);
	...
	float denom = (total_inside + total_outside) * (total_inside + total_outside);
	return (grad_inside * (2 * total_inside + total_outside) + grad_outside * total_inside) / denom;
    }
