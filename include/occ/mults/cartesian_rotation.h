...
/// Compute the second derivative of the rotation kernel w.r.t. angle-axis parameters.
///
/// Given M1 = dM/dp_k and M2 = dM/dp_l and M12 = d²M/dp_k dp_l, uses the product rule
/// to compute d²K/dp_k dp_l.
///
/// The kernel is:
///   K = sum_p mc * prod_{i,j} M(i,j)^p(i,j)
///
...
inline double rotation_kernel_second_derivative(int t, int u, int v,
                                                 int a, int b, int c,
                                                 const Mat3 &M,
                                                 const Mat3 &M1k, const Mat3 &M1l,
                                                 const Mat3 &M2kl) {
    double d2coeff = 0.0;
        ...
        for (int pxy = 0; pxy <= std::min(t - pxx, b); ++pxy) {
            ...
            for (int pyx = 0; pyx <= std::min(u, a - pxx); ++pyx) {
                ...
                for (int pyy = 0; pyy <= std::min(u - pyx, b - pxy); ++pyy) {
                    ...
                    // Two types of terms:
                    // 1. Replace one factor with M2kl (second derivative)
                    // 2. Replace two different factors with M1k and M1l

                    double deriv_term = 0.0;

                    // Term type 1: One factor replaced with M2kl
                    for (int ri = 0; ri < 3; ++ri) {
                        for (int rj = 0; rj < 3; ++rj) {
                            if (p[ri][rj] == 0) continue;
                            double prod = 1.0;
                    ...
                    // Term type 2: Two different factors replaced with M1k and M1l
                    for (int ri = 0; ri < 3; ++ri) {
                        for (int rj = 0; rj < 3; ++rj) {
                            if (p[ri][rj] == 0) continue;
                            for (int si = 0; si < 3; ++si) {
                                for (int sj = 0; sj < 3; ++sj) {
                                    if (si == ri && sj == rj) {
                                        // Same factor: need p[ri][rj] >= 2
                                        if (p[ri][rj] < 2) continue;
                                        double prod = 1.0;
                                        for (int i = 0; i < 3; ++i) {
                                            for (int j = 0; j < 3; ++j) {
                                                if (i == ri && j == rj) {
                                                    prod *= p[ri][rj] * (p[ri][rj] - 1)
                                                          * ipow(M(i, j), p[i][j] - 2)
                                                          * M1k(i, j) * M1l(i, j);
                                                } else {
                                                    prod *= ipow(M(i, j), p[i][j]);
...
/// Compute second derivative of lab-frame multipole w.r.t. angle-axis parameters.
///
/// Given M1k = dM/dp_k, M1l = dM/dp_l, and M2kl = d²M/dp_k dp_l,
/// computes d²(lab)/dp_k dp_l for the rotated multipole.
template <int MaxL>
void rotate_cartesian_multipole_second_derivative(
    const CartesianMultipole<MaxL> &body,
    const Mat3 &M,
    const Mat3 &M1k, const Mat3 &M1l,
    const Mat3 &M2kl,
    ...
    // Rank 0: second derivative is zero (scalar is rotation-invariant)
    d2_lab.data[0] = 0.0;
            ...
            for (int u = l - t; u >= 0; --u) {
                ...
                for (int a = l; a >= 0; --a) {
                    for (int b = l - a; b >= 0; --b) {
                        ...
                        double bv = body.data[hermite_index(a, b, c)];
                        if (bv == 0.0) continue;
                        val += rotation_detail::rotation_kernel_second_derivative(
                                   t, u, v, a, b, c, M, M1k, M1l, M2kl) * bv;
                    }
                }
                d2_lab.data[hermite_index(t, u, v)] = val;
            }
        }
    }
}
