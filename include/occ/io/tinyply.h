...
template <typename T, typename T2> inline T2 endian_swap(const T &v) noexcept {
  return v;
}
template <>
...
struct PlyFile::PlyFileImpl {
  ...
  size_t read_property_binary(const size_t &stride, void *dest,
                              size_t &destOffset, size_t destSize,
                              std::istream &is);
      ...
      for (auto &p : e.properties) {
        ...
        if (p.isList) {
          ...
          write_property_binary(
              os,
              (helper->data->buffer.get_const() + helper->cursor->byteOffset),
...
void PlyFile::PlyFileImpl::add_properties_to_element(
    const std::string &elementKey, const std::vector<std::string> propertyKeys,
    const Type type, const size_t count, const uint8_t *data,
...
void PlyFile::PlyFileImpl::parse_data(std::istream &is, bool firstPass) {
  std::function<void(PropertyLookup & f, const PlyProperty &p, uint8_t *dest,
                     size_t &destOffset, size_t destSize, std::istream &is)>
      read;
  ...
  if (isBinary) {
    read = [this, &listSize, &dummyCount, &read_list_binary](
               ...
               size_t &destOffset, size_t destSize, std::istream &_is) {
      if (!p.isList) {
        return read_property_binary(f.prop_stride, dest + destOffset,
                                    destOffset, destSize, _is);
      }
    ...
    skip = [this, &listSize, &dummyCount,
            &read_list_binary](PropertyLookup &f, const PlyProperty &p,
                               std::istream &_is) noexcept {
      if (!p.isList) {
    ...
    read = [this, &listSize, &dummyCount](
      ...
      if (!p.isList) {
        ...
        read_property_ascii(p.listType, f.list_stride, &listSize, dummyCount,
                            sizeof(listSize), _is); // the list size
    ...
    skip = [this, &listSize, &dummyCount,
            &skip_ascii_buffer](PropertyLookup &f, const PlyProperty &p,
                                std::istream &_is) noexcept {
      skip_ascii_buffer.clear();
      if (p.isList) {
        dummyCount = 0;
        read_property_ascii(
            p.listType, f.list_stride, &listSize, dummyCount, sizeof(listSize),
            _is); // the list size (does not count for memory alloc)
        for (size_t i = 0; i < listSize; ++i)
        ...
        if (!lookup.skip) {
          ...
          if (firstPass) {
            ...
            read(lookup, property, helper->data->buffer.get(),
                 helper->cursor->byteOffset, destSize, is);
          }
        } else {
