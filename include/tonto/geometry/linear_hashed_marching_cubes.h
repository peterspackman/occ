#pragma once
#include <tonto/geometry/linear_hashed_octree.h>
...
private:
    ...
    template<typename S>
    LinearHashedOctree<float> build_octree(S &source)
    {
                         ...
                         E &extract_fn)
    {
        ...
        for(const auto &kv : primal_vertices)
        {
            ...
            march_one_cube(duals, dual_distances, index_map, indices, base_index, extract_fn);
        }
    ...
    template<typename E>
    void march_one_cube(
        const std::array<MIndex, 8> &nodes,
