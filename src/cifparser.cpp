...
void CifParser::extract_atom_sites(const gemmi::cif::Loop &loop)
{
    ...
    {
        AtomData atom;
        if(label_idx >= 0) atom.site_label = loop.val(i, label_idx);
        if(symbol_idx >= 0) atom.element = loop.val(i, symbol_idx);
...
void CifParser::extract_cell_parameter(const gemmi::cif::Pair &pair)
{
    ...
    else if(tag == "_cell_angle_alpha") m_cell.alpha = craso::util::deg2rad(gemmi::cif::as_number(pair.back()));
    else if(tag == "_cell_angle_beta") m_cell.beta = craso::util::deg2rad(gemmi::cif::as_number(pair.back()));
    else if(tag == "_cell_angle_gamma") m_cell.gamma = craso::util::deg2rad(gemmi::cif::as_number(pair.back()));
}
...
std::optional<craso::crystal::Crystal> CifParser::parse_crystal(const std::string& filename)
{
    try {
        ...
        for(const auto& item: block.items)
        {
            if(item.type == gemmi::cif::ItemType::Pair) {
                if(item.has_prefix("_cell")) extract_cell_parameter(item.pair);
            }
            if(item.type == gemmi::cif::ItemType::Loop) {
                if(item.has_prefix("_atom_site")) extract_atom_sites(item.loop);
                else if(item.has_prefix("_symmetry_equiv_pos")) extract_symmetry_operations(item.loop);
        ...
        craso::crystal::UnitCell uc(m_cell.a, m_cell.b, m_cell.c, m_cell.alpha, m_cell.beta, m_cell.gamma);
