...
std::string chemical_formula_from_atoms(const std::vector<Atom> &atoms) {
  ...
  for (const auto &a : atoms)
    elements.emplace_back(a.atomic_number);
...
void register_enums_and_small_types(lua_State *L) {
  lb::getGlobalNamespace(L)
      ...
      .addProperty(
          "Dipole", +[]() { return static_cast<int>(cint::Operator::dipole); })
      .addProperty(
          "Quadrupole",
      ...
      .addProperty(
          "Rinv", +[]() { return static_cast<int>(cint::Operator::rinv); })
      .endNamespace()
...
void register_basis(lua_State *L) {
  lb::getGlobalNamespace(L)
      ...
      .addProperty(
          "origin", +[](const Shell *s) -> Vec3 { return s->origin; })
      .addProperty(
          "exponents", +[](const Shell *s) -> Vec { return s->exponents; })
      .addProperty(
  ...
  lb::getGlobalNamespace(L)
      ...
      .addFunction(
          ...
          +[](const MolecularOrbitals *mo) {
            return fmt::format("<MolecularOrbitals kind={} nao={} nalpha={} "
                               "nbeta={}>",
      ...
      .addFunction(
          "save", +[](Wavefunction *w,
                      const std::string &filename) { w->save(filename); })
      .addFunction(
      ...
      .addFunction(
          ...
          +[](const Wavefunction *wfn, const lb::LuaRef &points,
            ...
            const int d =
                derivatives.isNumber() ? derivatives.unsafe_cast<int>() : 0;
            return mat_to_table(points.state(),
...
void register_hf_and_scf(lua_State *L) {
  ...
  lb::getGlobalNamespace(L)
      ...
      .addPropertyReadWrite("incremental_fock_threshold",
      ...
      .addStaticFunction(
          "with_kind", +[](HartreeFock &hf,
                           SpinorbitalKind kind) { return new HF(hf, kind); })
      ...
      .addFunction(
          ...
          +[](HF *scf, const lb::LuaRef &V_ext_table, double nuclear_energy,
      ...
      .addFunction(
          ...
          +[](HartreeFock *hf, lua_State *S) {
            return mat_to_table(S, hf->compute_nuclear_attraction_matrix());
          })
      ...
      .addFunction("nuclear_repulsion", &HartreeFock::nuclear_repulsion_energy)
  ...
  lb::getGlobalNamespace(L)
      ...
      .addStaticFunction(
          ...
          +[](PointChargeList charges, std::vector<double> molecular_charges,
            ...
            return new WolfPointChargePotential{std::move(charges),
...
void register_hessian_and_vibrations(lua_State *L) {
  ...
  lb::getGlobalNamespace(L)
      ...
      .addProperty("use_acoustic_sum_rule", &HessEval::use_acoustic_sum_rule)
      .addFunction(
          "nuclear_repulsion",
          +[](HessEval *h) { return h->nuclear_repulsion(); })
      // HessianEvaluator::operator() takes a Wavefunction in this version
      // of occ, not a MolecularOrbitals directly.
      .addFunction(
          "compute",
          +[](HessEval *h, const Wavefunction &wfn) { return (*h)(wfn); })
      .addFunction(
          "__tostring",
          +[](const HessEval *h) {
            return fmt::format(
      ...
      .addProperty(
          "normal_modes", +[](const VM *v) -> Mat { return v->normal_modes; })
      .addProperty(
          "mass_weighted_hessian",
          +[](const VM *v) -> Mat { return v->mass_weighted_hessian; })
      .addProperty(
          "hessian", +[](const VM *v) -> Mat { return v->hessian; })
      ...
      .addFunction(
          "normal_modes_string_default",
          +[](const VM *v) { return v->normal_modes_string(0.1); })
      .addProperty(
          "get_all_frequencies",
...
void register_integral_engines(lua_State *L) {
  lb::getGlobalNamespace(L)
      ...
      .addFunction(
          "set_auxiliary_basis",
          +[](IntegralEngine *e, const std::vector<Shell> &basis, bool dummy) {
