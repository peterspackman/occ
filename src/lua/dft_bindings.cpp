  ...
  lb::getGlobalNamespace(L)
      .beginNamespace("occ")
        ...
        .beginClass<GridSettings>("GridSettings")
          ...
          .addPropertyReadWrite("radial_precision",
                                &GridSettings::radial_precision)
          .addFunction("__tostring", +[](const GridSettings *s) {
            return fmt::format(
        ...
        .beginClass<KS>("KS")
          ...
          .addStaticFunction(
              "new_restricted",
              +[](DFT &dft) {
                return new KS(dft, SpinorbitalKind::Restricted);
              })
          ...
          .addFunction(
              ...
              +[](KS *scf, const lb::LuaRef &V_ext_table, double nuc_e,
                ...
                for (int i = 0; i < n; ++i) {
                  ...
                  for (int j = 0; j < n; ++j) V(i, j) = lua_get_num(row, j + 1);
                }
          ...
          .addFunction("__tostring", +[](const KS *ks) {
        ...
        .beginClass<DFT>("DFT")
          ...
          .addFunction(
              ...
              +[](DFT *dft, const std::string &name,
                  const lb::LuaRef &threshold) {
                const double t = threshold.isNumber()
                                      ? threshold.unsafe_cast<double>()
                                      : 1e-4;
          ...
          .addFunction("compute_gradient",
                       +[](DFT *dft, const MolecularOrbitals &mo) {
                         occ::qm::GradientEvaluator<DFT> grad(*dft);
                         return grad(mo);
                       })
          .addFunction("hessian_evaluator",
                       +[](DFT *dft) { return HessEvalDFT(*dft); })
          .addFunction("__tostring", +[](const DFT *dft) {
        ...
        .beginClass<HessEvalDFT>("HessianEvaluatorDFT")
          ...
          .addProperty("use_acoustic_sum_rule",
                       &HessEvalDFT::use_acoustic_sum_rule)
          ...
          .addFunction("__tostring", +[](const HessEvalDFT *h) {
            return fmt::format(
        ...
        .beginClass<occ::xdm::XDM::Parameters>("XDMParameters")
          ...
          .addFunction("__tostring",
                       +[](const occ::xdm::XDM::Parameters *p) {
                         return fmt::format("<XDMParameters a1={:.4f} a2={:.4f}>",
                                            p->a1, p->a2);
                       })
        .endClass()
        ...
        .beginClass<occ::xdm::XDM>("XDM")
          ...
          .addFunction("forces",
                       +[](occ::xdm::XDM *x, const MolecularOrbitals &mo) {
                         return x->forces(mo);
                       })
          .addProperty("moments",
                       +[](const occ::xdm::XDM *x) { return x->moments(); })
          .addProperty(
              "hirshfeld_charges",
              +[](const occ::xdm::XDM *x) { return x->hirshfeld_charges(); })
          .addProperty("atom_volume",
          ...
          .addFunction("__tostring", +[](const occ::xdm::XDM *x) {
