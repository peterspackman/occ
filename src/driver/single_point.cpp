...
Wavefunction run_mp2_method(const Wavefunction& scf_wfn, const OccInput& config) {
  using occ::qm::MP2;
  ...
  MP2 mp2 = [&]() {
    if (!config.basis.ri_basis.empty()) {
      occ::log::info("Method: RI-MP2 (auxiliary basis: {})", config.basis.ri_basis);
      auto aux_basis = load_basis_set(config.geometry.molecule(), 
  ...
  occ::log::info("SCF energy:                       {: 20.12f}", scf_wfn.energy.total);
  occ::log::info("MP2 correlation energy:           {: 20.12f}", correlation_energy);
  occ::log::info("MP2 total energy:                 {: 20.12f}", total_mp2_energy);
  
  const auto& results = mp2.results();
  occ::log::debug("Same-spin correlation:            {: 20.12f}", results.same_spin_correlation);
  occ::log::debug("Opposite-spin correlation:        {: 20.12f}", results.opposite_spin_correlation);
  occ::log::debug("SCS-MP2 correlation energy:       {: 20.12f}", results.scs_mp2_correlation);
  occ::log::debug("SCS-MP2 total energy:             {: 20.12f}", scf_wfn.energy.total + results.scs_mp2_correlation);
