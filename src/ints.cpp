...
MatRM compute_2body_fock(const BasisSet &obs,
...
std::pair<MatRM, MatRM> compute_JK(const BasisSet &obs,
                                   const shellpair_list_t &shellpair_list,
...
std::pair<MatRM, MatRM> compute_2body_fock_unrestricted(
  ...
  auto engine_precision =
      std::min(fock_precision / std::max(max_coeff_a, max_coeff_b),
               std::numeric_limits<double>::epsilon()) /
      max_nprim4;
  assert(engine_precision > max_engine_precision &&
         "using precomputed shell pair data limits the max engine precision"
        ...
        for (auto s3 = 0; s3 <= s1; ++s3) {
          ...
          for (const auto &s4 : shellpair_list.at(s3)) {
            ...
            const auto Dnorm1234_a =
                do_schwarz_screen
                    ? std::max(D_shblk_norm_a(s1, s4),
                               std::max(D_shblk_norm_a(s2, s4),
                                        std::max(D_shblk_norm_a(s3, s4),
                                                 Dnorm123_a)))
                    : 0.;
            const auto Dnorm1234_b =
                do_schwarz_screen
                    ? std::max(D_shblk_norm_b(s1, s4),
                               std::max(D_shblk_norm_b(s2, s4),
                                        std::max(D_shblk_norm_b(s3, s4),
                                                 Dnorm123_b)))
                    : 0.;
...
std::pair<MatRM, MatRM> compute_JK_general(
    const BasisSet &obs, const shellpair_list_t &shellpair_list,
    const shellpair_data_t &shellpair_data, const MatRM &D,
    double precision, const MatRM &Schwarz) {
