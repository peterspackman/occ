...
PromoleculeDensity::PromoleculeDensity(const occ::core::Molecule &mol,
  ...
  FMat3N positions =
      (mol.positions().array() * occ::units::ANGSTROM_TO_BOHR).cast<float>();
  initialize_interpolators(elements, positions);
}

void PromoleculeDensity::initialize_interpolators(
    Eigen::Ref<const IVec> elements, Eigen::Ref<const FMat3N> positions) {
  ...
  for (const auto &kv : tmp_map) {
    m_atom_interpolators.push_back(
        {interpolators.at(kv.first), positions(Eigen::all, kv.second)});
    m_atom_interpolators.back().threshold =
        m_atom_interpolators.back().interpolator.find_threshold(1e-8);
  }
}

float PromoleculeDensity::maximum_distance_heuristic(float value,
                                                     float buffer) const {
  float result{0.0};

  for (const auto &[interp, coords, threshold] : m_atom_interpolators) {
