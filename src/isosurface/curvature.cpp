...
SurfaceCurvature calculate_curvature(const std::vector<float> &mean,
    ...
    for (int i = 0; i < result.mean.size(); i++) {
        float delta = std::sqrt(std::max(result.mean(i) * result.mean(i) - result.gaussian(i), 0.0f));
    ...
    fmt::print("Shape index range: {} {} {}\n", result.shape_index.minCoeff(), result.shape_index.mean(), result.shape_index.maxCoeff());
