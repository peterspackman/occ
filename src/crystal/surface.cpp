...
void Surface::print() const {
  occ::log::info("({} {} {}) spacing = {:.3f} area = {:.3f}", m_hkl.h, m_hkl.k,
                 m_hkl.l, d(), area());
  occ::log::debug(
...
Surface::find_molecule_cell_translations(const std::vector<Molecule> &mols,
  ...
  occ::log::debug("[{:9.3f}, {:9.3f}, {:9.3f}]", cb_in_sb(0, 0), cb_in_sb(0, 1),
                  cb_in_sb(0, 2));
  occ::log::debug("[{:9.3f}, {:9.3f}, {:9.3f}]", cb_in_sb(1, 0), cb_in_sb(1, 1),
                  cb_in_sb(1, 2));
  occ::log::debug("[{:9.3f}, {:9.3f}, {:9.3f}]", cb_in_sb(2, 0), cb_in_sb(2, 1),
                  cb_in_sb(2, 2));

  occ::log::debug("Surface basis in Crystal coordinates");
  occ::log::debug("[{:9.3f}, {:9.3f}, {:9.3f}]", sb_in_cb(0, 0), sb_in_cb(0, 1),
                  sb_in_cb(0, 2));
  occ::log::debug("[{:9.3f}, {:9.3f}, {:9.3f}]", sb_in_cb(1, 0), sb_in_cb(1, 1),
                  sb_in_cb(1, 2));
  occ::log::debug("[{:9.3f}, {:9.3f}, {:9.3f}]", sb_in_cb(2, 0), sb_in_cb(2, 1),
                  sb_in_cb(2, 2));
  ...
  HKL lower = HKL::floor(lower_vec_crystal.array() - 2.0);
  ...
  for (int h = lower.h; h <= upper.h; h++) {
    for (int k = lower.k; k <= upper.k; k++) {
      for (int l = lower.l; l <= upper.l; l++) {
        ...
        Vec3 cart_shift = m_crystal_unit_cell.to_cartesian(frac_t_crystal);
...
unique_counts_from_dimers(const SurfaceCutResult::DimerCounts &counts,
  ...
  for (const auto &mol_neighbors : dimers.molecule_neighbors) {
    for (const auto &pair : mol_neighbors) {
      ...
      max_asym_idx =
          std::max(max_asym_idx, pair.dimer.a().asymmetric_molecule_idx());
    }
  }
  ...
  for (const auto &mol : result.molecules) {
    ...
    for (int atom_idx = 0; atom_idx < elements.size(); atom_idx++) {
      lines.push_back(fmt::format(
          "{} {:12.6f} {:12.6f} {:12.6f} {} {}", elements[atom_idx].symbol(),
          mol_pos_cart(0, atom_idx), mol_pos_cart(1, atom_idx),
          mol_pos_cart(2, atom_idx), t_index, index));
    }
    ...
    occ::log::debug("mol in surface index = {}, unit cell index = {}", t_index,
                    index);
    occ::log::debug("Fractional center: {:12.6f} {:12.6f} {:12.6f}", frac_c(0),
                    frac_c(1), frac_c(2));
    occ::log::debug("Shift (hkl): {} {} {}", uc_shift[0], uc_shift[1],
                    uc_shift[2]);
    occ::log::debug("Shift (cart): {:12.6f} {:12.6f} {:12.6f}", translation(0),
    ...
    for (int neighbor_index = 0; neighbor_index < neighbors.size();
      ...
      if ((c_a - mol.centroid()).norm() > 1e-3) {
        occ::log::error("check for mol_a translated positions matching failed");
      }
      if ((std::abs(r - (c_a - c_b).norm()) > 1e-3)) {
        occ::log::error("check for translated distance in Surface cut failed");
      }
