...
template<typename T>
void loop_over_miller_indices(T& func, const Crystal &c, double dmin, double dmax = 1.0, bool unique = true)
{
    ...
    for(m.l = - limits.l; m.l <= limits.l; m.l++)
    {
        if(!unique || rasu.is_in(m))
        {
            ...
            if(d > dmin && d < dmax) func(m);
        }
    }
}

Surface::Surface(const MillerIndex &miller, const Crystal& crystal) : m_hkl{miller}, 
    m_lattice(crystal.unit_cell().direct()),
...
double Surface::d() const
{
    return m_hkl.d(m_reciprocal_lattice);
}

std::array<double, 3> Surface::dipole() const
{
    return {};
}
...
std::vector<Surface> generate_surfaces(const Crystal &c, double d_max)
{
    ...
    auto f = [&](const MillerIndex& m) {
        result.emplace_back(Surface(m, c));
    };
    loop_over_miller_indices(f, c, 0.05, 0.5);
    std::sort(result.begin(), result.end(), [](const Surface &a, const Surface &b) { return a.depth() > b.depth(); });
    return result;
}
