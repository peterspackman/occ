...
SymOp::SymOp(const Mat3& rotation, const Vec3& translation) : transformation(Mat4::Zero()) {
...
SymOp SymOp::from_axis_angle(const Vec3& axis, double angle, const Vec3& origin) {
...
SymOp SymOp::rotoreflection(const Vec3& axis, double angle, const Vec3& origin) {
...
IVec smallest_off_axis_group(const occ::chem::Molecule& mol, const Vec3& axis) {
    ...
    */
    ...
    IVec mask(N);
    robin_hood::unordered_map<int, int> group_size;
    for(int i = 0; i < N; i++) {
        int el = els(i);
    ...
    for(int i = 0; i < N; i++) if(mask(i) != min_el) mask(i) = 0;
    return mask;
}
...
void MolecularPointGroup::init() {
    ...
    rotor_type = occ::core::rotor::classify(eigenvalues(0), eigenvalues(1), eigenvalues(2), etol);
...
void MolecularPointGroup::find_spherical_axes() {
    ...
    */
    ...
    for(int i = 0; i < nums.rows(); i++) {
        int el = nums(i);
        if(!count.contains(el)) count[el] = 1;
        else count[el]++;
    }
    const robin_hood::pair<int, int>& min_el = *std::min_element(
        ...
        for(int j = i + 1; j < N; j++) {
            for(int k = j + 1; k < N; k++) {
                ...
                for(int c = 0; c < 3; c++) {
                    ...
                    if(test_axis.norm() < tol) continue;
                ...
                Vec3 test_axis = (coords.col(j) - coords.col(i)).cross(
                        coords.col(k) - coords.col(i));
                if(test_axis.norm() < tol) continue;
                for(int r = 3; r < 6; r++) {
                    if(rot_found[r]) continue;
...
PointGroup dihedral_group(int r, MirrorType m) {
    if(m == MirrorType::H) {
        switch(r) {
            ...
            case 7: return PointGroup::D7h;
            case 8: return PointGroup::D8h;
        }
    }
    if(m == MirrorType::D) {
        switch(r) {
            ...
            case 7: return PointGroup::D7d;
            case 8: return PointGroup::D8d;
        }
    }
    switch(r) {
        ...
        case 7: return PointGroup::D7;
        case 8: return PointGroup::D8;
    }
...
PointGroup cyclic_group(int r, MirrorType m) {
    ...
    if(m == MirrorType::V) {
        switch(r) {
            case 2: return PointGroup::C2v;
            case 3: return PointGroup::C3v;
            case 4: return PointGroup::C4v;
            case 5: return PointGroup::C5v;
            case 6: return PointGroup::C6v;
        }
    }
    switch(r) {
        case 2: return PointGroup::C2;
        case 3: return PointGroup::C3;
        case 4: return PointGroup::C4;
        case 5: return PointGroup::C5;
        case 6: return PointGroup::C6;
        case 8: return PointGroup::C8;
    }
...
bool MolecularPointGroup::check_perpendicular_r2_axis(const Vec3& axis) {
    ...
    */
    ...
    const auto& positions = centered_molecule.positions();
    for(int i = 0; i < N; i++) {
        if(min_set(i) < 1) continue;
        for(int j = i + 1; j < N; j++) {
            if(min_set(j) < 1) continue;
...
MirrorType MolecularPointGroup::find_mirror(const Vec3 &axis) {
    ...
    for(int i = 0; i < N; i++) {
        for(int j = i + 1; j < N; j++) {
            ...
            if(normal.dot(axis) >= tol) continue;

            SymOp op = SymOp::reflection(normal);
            if(!is_valid_symop(op, positions)) continue;
...
bool MolecularPointGroup::check_R2_axes_asym() {
    ...
    for(int i = 0; i < 3; i++) {
        ...
        if(is_valid_symop(op, centered_molecule.positions())) {
            symmetry_operations.push_back(op);
            rotational_symmetry.push_back(std::make_pair(eigenvectors.col(i), 2));
            found = true;
        }
    }
    return found;
...
void MolecularPointGroup::init_cyclic() {
    ...
    MirrorType mirror_type = find_mirror(m.first);
    if(mirror_type == MirrorType::None) {
        if(is_valid_symop(SymOp::rotoreflection(m.first, 180.0/m.second), centered_molecule.positions())) {
            switch(m.second) {
                case 2: group = PointGroup::S4; return;
                case 3: group = PointGroup::S6; return;
                case 4: group = PointGroup::S8; return;
            }
        }
    }
...
void MolecularPointGroup::init_symmetric_top() {
    Eigen::Index ind{1};
    if(std::abs(eigenvalues(0) - eigenvalues(1)) < etol) ind = 2;
    else if(std::abs(eigenvalues(1) - eigenvalues(2)) < etol) ind = 0;
    ...
    else init_no_rotational_symmetry();
}
...
const char* MolecularPointGroup::point_group_string() const {
    switch (group) {
        ...
        case PointGroup::C8: return "C8";
        case PointGroup::Coov: return "Coov";
        case PointGroup::Dooh: return "Dooh";
        ...
        case PointGroup::D7: return "D7";
        case PointGroup::D8: return "D8";
        ...
        case PointGroup::D7h: return "D7h";
        case PointGroup::D8h: return "D8h";
        ...
        case PointGroup::D7d: return "D7d";
        case PointGroup::D8d: return "D8d";
...
const char* MolecularPointGroup::description() const {
    switch (group) {
        case PointGroup::C1: return "C1";
        case PointGroup::Cs: return "mirror plane";
        case PointGroup::Ci: return "inversion center";
        case PointGroup::Coov: return "linear";
        case PointGroup::Dooh: return "linear + inversion center";
        case PointGroup::C2: return "open book geometry";
        case PointGroup::C3: return "propeller";
        case PointGroup::C2v: return "angular, see-saw, T-shape";
        case PointGroup::C3v: return "trigonal pyramidal";
        case PointGroup::C4v: return "square pyramidal";
        case PointGroup::C5v: return "milking stool";
        case PointGroup::D2: return "twist";
        case PointGroup::D2h: return "planar + inversion center";
        case PointGroup::D3h: return "trigonal planar, trigonal bipyramidal";
        case PointGroup::D4h: return "square planar";
        case PointGroup::D5h: return "pentagonal";
        case PointGroup::D6h: return "hexagonal";
        case PointGroup::D7h: return "heptagonal";
        case PointGroup::D8h: return "octagonal";
        case PointGroup::D2d: return "90 degree twist";
        case PointGroup::D3d: return "60 degree twist";
        case PointGroup::D4d: return "45 degree twist";
        case PointGroup::D5d: return "36 degree twist";
        case PointGroup::S4: return "S4";
        case PointGroup::Td: return "tetrahedral";
        case PointGroup::Th: return "pyritohedron";
        case PointGroup::Oh: return "octahedral, cubic";
        case PointGroup::Ih: return "icosahedral, dodecahedral";
        default: return "none";
    }
}
