...
double element_sigma(const SMDSolventParameters &params, int z) {
  if (params.is_water) {
    ...
    return sigma_n(z) * params.refractive_index_293K +
           sigma_alpha(z) * params.acidity + sigma_beta(z) * params.basicity;
  }
}
...
double element_pair_prefactor(const SMDSolventParameters &params,
  ...
  if (params.is_water) {
    ...
    return sigma_n(p) * params.refractive_index_293K +
           sigma_alpha(p) * params.acidity + sigma_beta(p) * params.basicity;
  }
}
...
Vec atomic_surface_tension(const SMDSolventParameters &params, const IVec &nums,
  ...
  for (int i = 0; i < N; i++) {
    ...
    case 7: {
      ...
      double nc3_sigma =
          detail::element_pair_prefactor(params, detail::ElementPair::N_C3);
      if (nc3_sigma != 0.0) {
...
Vec intrinsic_coulomb_radii(const std::vector<core::Atom> &atoms,
                            const SMDSolventParameters &params) {
...
Vec cds_radii(const std::vector<core::Atom> &atoms,
              const SMDSolventParameters &params) {
  IVec nums(atoms.size());
  for (int i = 0; i < atoms.size(); i++)
    nums(i) = atoms[i].atomic_number;
