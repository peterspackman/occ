    ...
    while (!m_faceList.empty()) {
        ...
        while (m_possiblyVisibleFaces.size()) {
                    ...
                    for (auto heIndex : m_mesh.getHalfEdgeIndicesOfFace(pvf)) {
                        if (m_mesh.m_halfEdges[heIndex].m_opp !=
                            ...
                            m_possiblyVisibleFaces.emplace_back(
                                m_mesh
                                    .m_halfEdges[m_mesh.m_halfEdges[heIndex]
                                                     .m_opp]
                                    .m_face,
                                heIndex);
                        }
            ...
            const auto halfEdges = m_mesh.getHalfEdgeIndicesOfFace(
                m_mesh
                    .m_faces[m_mesh.m_halfEdges[faceData.m_enteredFromHalfEdge]
                                 .m_face]);
        ...
        if (!reorderHorizonEdges(m_horizonEdges)) {
            ...
            auto it =
                std::find(tf.m_pointsOnPositiveSide->begin(),
        ...
        for (size_t i = 0; i < horizonEdgeCount; i++) {
            ...
            const Eigen::Matrix<T, 3, 1> planeNormal =
                mathutils::triangle_normal(m_vertexData.col(A),
                                           m_vertexData.col(B), activePoint);
            newFace.m_P = Plane<T>(planeNormal, activePoint);
            newFace.m_he = AB;

            m_mesh.m_halfEdges[CA].m_opp =
                m_newHalfEdgeIndices[i > 0 ? i * 2 - 1
...
bool QuickHull<T>::reorderHorizonEdges(std::vector<size_t> &horizonEdges) {
    ...
    for (size_t i = 0; i < horizonEdgeCount - 1; i++) {
        const size_t endVertex =
            m_mesh.m_halfEdges[horizonEdges[i]].m_endVertex;
...
template <typename T> void QuickHull<T>::setupInitialTetrahedron() {
    ...
    if (vertexCount <= 4) {
        ...
        const Eigen::Matrix<T, 3, 1> N = mathutils::triangle_normal(
            Eigen::Matrix<T, 3, 1>(m_vertexData.col(v[0])),
            m_vertexData.col(v[1]), m_vertexData.col(v[2]));
    ...
    for (size_t i = 0; i < vCount; i++) {
        const T distToRay = mathutils::getSquaredDistanceBetweenPointAndRay(
            m_vertexData.col(i), r);
        if (distToRay > maxD) {
            maxD = distToRay;
            maxI = i;
        }
    ...
    if (maxD == m_epsilon) {
        ...
        m_vertexData = Eigen::Matrix<T, 3, Eigen::Dynamic>(
            3, m_planarPointCloudTemp.size());
        for (int i = 0; i < m_vertexData.cols(); i++) {
    ...
    for (auto &f : m_mesh.m_faces) {
        ...
        const Eigen::Matrix<T, 3, 1> N1 =
            mathutils::triangle_normal(va, vb, vc);
