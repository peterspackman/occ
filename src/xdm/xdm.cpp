...
std::pair<double, Mat3N> xdm_dispersion_energy(const std::vector<occ::core::Atom> &atoms,
...
double XDM::energy(const occ::qm::MolecularOrbitals &mo) {
    ...
    std::tie(m_energy, m_forces) = xdm_dispersion_energy(m_basis.atoms(), m_moments, m_volume, m_volume_free);
    return m_energy;
}
