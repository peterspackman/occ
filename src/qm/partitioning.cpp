...
Vec mulliken(const AOBasis &basis, const Mat &D, Eigen::Ref<const Mat> op) {
  ...
  if constexpr (kind == R) {
    ...
    Vec pop = ((block::aa(D) + block::ab(D) + block::ba(D) + block::bb(D)) * op)
                  .diagonal();
    for (int u = 0; u < nbf; u++) {
      N(bf2atom[u]) += pop(u);
    }
...
Vec mulliken_partition(const AOBasis &basis, const MolecularOrbitals &mo,
  ...
  switch (mo.kind) {
  case U:
    return impl::mulliken<U>(basis, mo.D, op);
  case G:
    return impl::mulliken<G>(basis, mo.D, op);
  default:
    return impl::mulliken<R>(basis, mo.D, op);
  }
}
