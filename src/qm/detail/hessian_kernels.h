...
void evaluate_two_center_hess(Lambda &f, IntEnv &env, const AOBasis &basis,
                              int thread_id = 0) {
  using Result = IntegralEngine::IntegralResult<2>;
  occ::qm::cint::Optimizer opt(env, op, 2, 2);
  auto nthreads = occ::parallel::get_num_threads();
  auto bufsize = env.buffer_size_1e(op, 2);
  ...
  for (int p = 0, pq = 0; p < nsh; p++) {
    ...
    for (int q = 0; q <= p; q++) {
      ...
      Result args{thread_id,
                  idxs,
                  {bf1, bf2},
                  env.two_center_helper_hess<op, kind>(
...
template <Op op, ShellKind kind, typename Lambda>
void evaluate_two_center_with_shellpairs_hess(Lambda &f, IntEnv &env,
                                              const AOBasis &basis,
...
template <Op op, ShellKind kind, typename Lambda>
void evaluate_four_center_hess(Lambda &f, IntEnv &env, const AOBasis &basis,
                               const ShellPairList &shellpairs,
                               const Mat &Dnorm = Mat(),
  ...
  auto nthreads = occ::parallel::get_num_threads();
  occ::qm::cint::Optimizer opt(env, Op::coulomb, 4, 2);
  auto buffer = std::make_unique<double[]>(env.buffer_size_2e(2));
        ...
        for (const auto s : shellpairs[r]) {
...

          {
            Result args{
                ...
                env.four_center_helper_hess<Op::coulomb, kind>(
...
template <class Func>
inline void
four_center_inner_loop_hess(Func &store,
                            const IntegralEngine::IntegralResult<4> &args,
                            const Mat &D, MatSix &dest) {
    ...
    for (auto f2 = 0; f2 < args.dims[2]; ++f2) {
      ...
      for (auto f1 = 0; f1 < args.dims[1]; ++f1) {
        ...
        for (auto f0 = 0; f0 < args.dims[0]; ++f0, ++f0123) {
          ...
          // MatSix components: xx, yy, zz, xy, xz, yz
          store(D, dest.xx, bf0, bf1, bf2, bf3, args.buffer[f0123] * scale);
          store(D, dest.yy, bf0, bf1, bf2, bf3,
                args.buffer[f0123 + num_elements] * scale);
          store(D, dest.zz, bf0, bf1, bf2, bf3,
                args.buffer[f0123 + 2 * num_elements] * scale);
          store(D, dest.xy, bf0, bf1, bf2, bf3,
                args.buffer[f0123 + 3 * num_elements] * scale);
          store(D, dest.xz, bf0, bf1, bf2, bf3,
                args.buffer[f0123 + 4 * num_elements] * scale);
          store(D, dest.yz, bf0, bf1, bf2, bf3,
                args.buffer[f0123 + 5 * num_elements] * scale);
        }
      }
    }
  }
