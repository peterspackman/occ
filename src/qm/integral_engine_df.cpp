...
Eigen::Tensor<double, 4> IntegralEngineDF::four_center_integrals_tensor() const {
  occ::timing::start(occ::timing::category::df);
  ...
  occ::log::debug("Basis functions: {} AO, {} auxiliary, {} threads", nbf, naux, nthreads);
      ...
      for (size_t rho = 0; rho < nbf; ++rho) {
        for (size_t sigma = 0; sigma < nbf; ++sigma) {
          ...
          for (size_t P = 0; P < naux; ++P) {
            const auto eri_P = Eigen::Map<const Mat>(m_integral_store.col(P).data(), nbf, nbf);
...
MP2Components IntegralEngineDF::compute_df_mp2_energy(const MolecularOrbitals& mo, const Vec& orbital_energies,
    ...
    for (size_t j = 0; j < orbital_spec.n_active_occ; ++j) {
      ...
      for (size_t a = 0; a < orbital_spec.n_active_virt; ++a) {
        for (size_t b = 0; b < orbital_spec.n_active_virt; ++b) {
          ...
          double denominator = orbital_energies(i_full) + orbital_energies(j_full) - 
          ...
          if (std::abs(denominator) > denominator_threshold) {
            ...
            opposite_spin_energy += 2.0 * integral_iajb * integral_iajb / denominator;
