...
Mat IntegralEngineDF::exchange(const MolecularOrbitals &mo) {
  ...
  if (!direct) {
    ...
    case U:
      return stored_exchange_kernel_u(m_integral_store, m_ao_engine.aobasis(),
                                      m_ao_engine.auxbasis(), mo, V_LLt);
    case G:
      return stored_exchange_kernel_g(m_integral_store, m_ao_engine.aobasis(),
                                      m_ao_engine.auxbasis(), mo, V_LLt);
    }
  ...
  if (!direct) {
    ...
    default: // Restricted
      return stored_coulomb_kernel_r(m_integral_store, m_ao_engine.aobasis(),
                                     m_ao_engine.auxbasis(), mo, V_LLt);
    case U:
      return stored_coulomb_kernel_u(m_integral_store, m_ao_engine.aobasis(),
                                     m_ao_engine.auxbasis(), mo, V_LLt);
    case G:
      return stored_coulomb_kernel_g(m_integral_store, m_ao_engine.aobasis(),
                                     m_ao_engine.auxbasis(), mo, V_LLt);
    }
  } else if (m_ao_engine.is_spherical()) {
    occ::qm::cint::Optimizer opt(m_ao_engine.env(), Op::coulomb, 3);
