...
MolecularOrbitals::MolecularOrbitals(const MolecularOrbitals &mo1,
  ...
  auto merge_mo_block =
      [](Eigen::Ref<Mat> dest, Eigen::Ref<Vec> dest_energies,
        ...
        auto [C_merged, energies_merged] =
            merge_molecular_orbitals(coeffs1, coeffs2, energies1, energies2);
  ...
  if (kind == R) {
    ...
    occ::log::debug("Merging alpha spin-unrestricted occupied orbitals, "
                    "sorted by energy");
    merge_mo_block(
        alpha_coeffs.leftCols(n_alpha), alpha_energies.topRows(n_alpha),
        alpha_mo1.leftCols(mo1_na), alpha_mo2.leftCols(mo2_na),
        alpha_mo1_energies.topRows(mo1_na), alpha_mo2_energies.topRows(mo2_na));
    // merge virtual orbitals
    ...
    occ::log::debug("Merging beta spin-unrestricted occupied orbitals, "
                    "sorted by energy");
    merge_mo_block(beta_coeffs.leftCols(n_beta), beta_energies.topRows(n_beta),
                   beta_mo1.leftCols(mo1_nb), beta_mo2.leftCols(mo2_nb),
    ...
    if (nv_ab > 0) {
      ...
      merge_mo_block(beta_coeffs.rightCols(nv_ab),
                     beta_energies.bottomRows(nv_ab), beta_mo1.rightCols(nv_a),
                     beta_mo2.rightCols(nv_b),
                     beta_mo1_energies.bottomRows(nv_a),
                     beta_mo2_energies.bottomRows(nv_b));
    }
  } else {
    throw std::runtime_error("Merging general spinorbitals not implemented");
  }
...
void MolecularOrbitals::update_occupied_orbitals() {
  ...
  occ::log::debug("Updating occupied orbitals, n_a = {}, n_b = {}", n_alpha,
                  n_beta);
  constexpr auto U = SpinorbitalKind::Unrestricted;
  constexpr auto G = SpinorbitalKind::General;
...
Mat MolecularOrbitals::energy_weighted_density_matrix() const {
  ...
  switch (kind) {
  case SpinorbitalKind::Unrestricted:
    return orb::weighted_density_matrix_unrestricted(C, energies.array() *
                                                            occupation.array());
    break;
  default:
    return orb::weighted_density_matrix_restricted(C, energies.array() *
                                                          occupation.array());
    break;
  }
  occ::timing::stop(occ::timing::category::la);
...
void MolecularOrbitals::rotate(const AOBasis &basis, const Mat3 &rotation) {
  ...
  if (basis.is_pure()) {
    rotation_matrices =
        occ::gto::spherical_gaussian_rotation_matrices(basis.l_max(), rotation);
  } else {
    rotation_matrices =
        occ::gto::cartesian_gaussian_rotation_matrices(basis.l_max(), rotation);
  }
...
void MolecularOrbitals::to_cartesian(const AOBasis &bspure,
  ...
  if (kind == SpinorbitalKind::Restricted) {
    ...
    auto [nrows, ncols] =
        occ::qm::matrix_dimensions<SpinorbitalKind::Unrestricted>(bscart.nbf());
...
void MolecularOrbitals::to_spherical(const AOBasis &bscart,
  ...
  if (kind == SpinorbitalKind::Restricted) {
    ...
    auto [nrows, ncols] =
        occ::qm::matrix_dimensions<SpinorbitalKind::Unrestricted>(bspure.nbf());
    Cnew = Mat::Zero(nrows, ncols);
  }
...
MolecularOrbitals MolecularOrbitals::symmetrically_orthonormalized(
  ...
  case U: {
    // alpha spin
    block::a(result.C).leftCols(n_occ) = symmetrically_orthonormalize_matrix(
        block::a(result.Cocc), overlap_matrix);
    // beta spin
    block::b(result.C).leftCols(n_occ) = symmetrically_orthonormalize_matrix(
        block::b(result.Cocc), overlap_matrix);

    if (n_virt > 0) {
      Mat Cvirt = result.C.rightCols(n_virt);

      block::a(result.C).rightCols(n_virt) =
          symmetrically_orthonormalize_matrix(block::a(Cvirt), overlap_matrix);
      block::b(result.C).rightCols(n_virt) =
          symmetrically_orthonormalize_matrix(block::b(Cvirt), overlap_matrix);
    }
    break;
  }
  case G: {
