...
Eigen::Tensor<double, 4> IntegralEngine::four_center_integrals_tensor(const Mat &Schwarz) const {
  using Result = IntegralEngine::IntegralResult<4>;
  const size_t n_ao = nbf();
  constexpr auto op = cint::Operator::coulomb;
  auto nthreads = occ::parallel::get_num_threads();
  
  occ::log::info("Computing AO integrals using parallel dense tensor with {} threads", nthreads);
  occ::log::info("AO basis size: {} functions", n_ao);
  occ::log::info("Using 8-fold symmetry storage - storing only unique integrals");
  if (Schwarz.size() > 0) {
  ...
  auto lambda = [&](int thread_id) {
    if (is_spherical()) {
      detail::evaluate_four_center<op, Shell::Kind::Spherical>(
          f, m_env, m_aobasis, m_shellpairs, Schwarz, Mat(), m_precision, thread_id);
    } else {
...
double IntegralEngine::get_integral_8fold_symmetry(const Eigen::Tensor<double, 4>& tensor,
