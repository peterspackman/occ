...
qm::MolecularOrbitals convert_order(const gto::AOBasis &basis,
...
qm::MolecularOrbitals to_gaussian_order(const gto::AOBasis &basis,
                                        const qm::MolecularOrbitals &mo) {
  if (basis.is_pure()) {
    return convert_order(basis, mo, [](const gto::AOBasis &b, const Mat &m) {
...
qm::MolecularOrbitals from_gaussian_order(const gto::AOBasis &basis,
                                          const qm::MolecularOrbitals &mo) {
 ...
 *
 */
Mat from_gaussian_order_cartesian(const occ::gto::AOBasis &basis,
                                  const Mat &mo) {
...
Mat to_gaussian_order_cartesian(const occ::gto::AOBasis &basis, const Mat &mo) {
  // no reordering should occur unless there are d, f, g, h etc. functions
  using occ::util::index_of;
  constexpr auto their_order = occ::gto::ShellOrder::Gaussian;
  if (basis.l_max() < 2)
...
Mat to_gaussian_order_spherical(const occ::gto::AOBasis &basis, const Mat &mo) {
  using occ::util::index_of;
...
Mat from_gaussian_order_spherical(const occ::gto::AOBasis &basis,
                                  const Mat &mo) {
  using occ::util::index_of;
  if (basis.l_max() < 1)
    return mo;
