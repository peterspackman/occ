...
void MoldenReader::parse_section(const std::string &section_name,
  ...
  if (section_name == "Title") {
    ...
    for (auto &shell : m_shells) {
      shell.kind = gto::Shell::Kind::Spherical;
    }
...
inline occ::gto::Shell parse_molden_shell(const std::array<double, 3> &position,
                                         bool pure, std::istream &stream,
                                         std::string &line_buffer) {
  std::getline(stream, line_buffer);
  occ::gto::Shell::Kind shell_kind =
      pure ? occ::gto::Shell::Kind::Spherical : occ::gto::Shell::Kind::Cartesian;
...
Mat MoldenReader::convert_mo_coefficients_from_molden_convention(
    const occ::gto::AOBasis &basis, const Mat &mo) const {
