...
SemiNumericalExchange::SemiNumericalExchange(
    ...
    m_overlap = occ::ints::compute_1body_ints<Operator::overlap>(
        m_basis, m_shellpair_list)[0];
    m_numerical_overlap = compute_overlap_matrix();
    occ::log::debug(
        "Max error |Sn - S|: {:12.8f}\n",
    ...
    for (const auto &atom_grid : m_atom_grids) {
        ...
        auto lambda = [&](int thread_id) {
            ...
            for (size_t block = 0; block < num_blocks; block++) {
                ...
                occ::gto::evaluate_basis(m_basis, m_atoms, pts_block, ao, 0);
                S.noalias() +=
                    ao.phi.transpose() *
        ...
        auto lambda = [&](int thread_id) {
            ...
            for (size_t block = 0; block < num_blocks; block++) {
                ...
                for (size_t pt = 0; pt < npt; pt++) {
                    ...
                    chgs[0] = {
                        1,
                        {pts_block(0, pt), pts_block(1, pt), pts_block(2, pt)}};
                    ...
                    for (size_t s1 = 0l, s12 = 0l; s1 != nshells; ++s1) {
                        ...
                        for (size_t s2 : m_shellpair_list.at(s1)) {
                            ...
                            if (s1 != s2) {
                                A.block(bf2, bf1, n2, n1) = buf_mat.transpose();
                            }
                        }
                    }
