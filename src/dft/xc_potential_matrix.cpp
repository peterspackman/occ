...
void xc_potential_matrix<Unrestricted, 1>(const DensityFunctional::Result &res,
    ...
    auto ga = rho.alpha().block(0, 1, npt, 3).array().colwise() *
                  (2 * res.vsigma.col(0).array()) +
              rho.beta().block(0, 1, npt, 3).array().colwise() *
                  res.vsigma.col(1).array();
    auto gb = rho.beta().block(0, 1, npt, 3).array().colwise() *
                  (2 * res.vsigma.col(2).array()) +
              rho.alpha().block(0, 1, npt, 3).array().colwise() *
                  res.vsigma.col(1).array();
