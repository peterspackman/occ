...
uint_fast16_t angular_order(uint_fast16_t n) {
    ...
    throw std::runtime_error(
        "Request number of angular points too exceeds 5810");
    // unreachable
    return 1;
}
...
RadialGrid generate_lmg_radial_grid(size_t atomic_number,
    ...
    for (int i = 0; i <= l_max; i++) {
        if (alpha_min[i] > 0.0) {
            r_outer = std::max(r_outer, lmg_outer(radial_precision,
                                                  alpha_min(i), i, 4.0 * br));
...
AtomGrid generate_atom_grid(size_t atomic_number, size_t max_angular_points,
    ...
    for (size_t i = 0; i < n_radial; i++) {
        ...
        result.weights.block(num_points, 0, lebedev.rows(), 1) =
            lebedev.col(3) * w;
...
AtomGrid MolecularGrid::generate_partitioned_atom_grid(size_t atom_idx) const {
    ...
    for (size_t i = 0; i < natoms; i++) {
        ...
        for (size_t j = 0; j < i; j++) {
            ...
            occ::Vec w =
                (grid_dists.col(i).array() - grid_dists.col(j).array()) /
                m_dists(i, j);
