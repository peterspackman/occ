    ...
    if (builder.cout_option == PipeOption::specific) {
        ...
        if (builder.cout_pipe == kBadPipeValue)
            throw std::invalid_argument(
                "Popen constructor: bad pipe value for cout");
    }
...
Popen::Popen(Popen &&other) { *this = std::move(other); }
...
#endif
bool Popen::terminate() { return send_signal(PSIGTERM); }
bool Popen::kill() { return send_signal(PSIGKILL); }

std::string ProcessBuilder::windows_command() { return this->command[0]; }
