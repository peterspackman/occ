...
struct cstring_vector {
    ...
    void push_back(nullptr_t) { m_list.push_back(nullptr); }
    void push_back(const char *str) {
        char *copy = str ? strdup(str) : nullptr;
        m_list.push_back(copy);
    }
    void push_back_owned(char *str) { m_list.push_back(str); }
    value_type &operator[](std::size_t index) { return m_list[index]; }
    char **data() { return &m_list[0]; }
...
Popen ProcessBuilder::run_command(const CommandLine &command) {
    ...
    if (cin_option == PipeOption::close)
        ...
        if (this->cin_pipe == kBadPipeValue) {
            throw std::invalid_argument(
                "ProcessBuilder: bad pipe value for cin");
        }
    ...
    if (cout_option == PipeOption::close)
        ...
        if (this->cout_pipe == kBadPipeValue) {
            throw std::invalid_argument(
                "ProcessBuilder: bad pipe value for cout");
        }
    ...
    if (cerr_option == PipeOption::close)
        ...
        if (this->cerr_pipe == kBadPipeValue) {
            throw std::invalid_argument(
                "ProcessBuilder: bad pipe value for cerr");
        }
    ...
    attributes_raii.setflags(flags);
    {
        ...
        */
        ...
        int ret = posix_spawn(&pid, args[0], actions.get(), &attributes,
                              &args[0], env);
        if (ret != 0)
