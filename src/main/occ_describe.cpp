...
std::string to_string(DescribeConfig::Descriptor desc) {
    switch(desc) {
	case DescribeConfig::Descriptor::Steinhardt: return "steinhardt";
    }
}
...
std::vector<DescribeConfig::Descriptor> DescribeConfig::descriptors() const {
    ...
    ankerl::unordered_dense::map<std::string, DescribeConfig::Descriptor> name2desc;
...
CLI::App *add_describe_subcommand(CLI::App &app) {
    ...
    desc->add_option("--descriptor",
	            config->descriptor_strings,
