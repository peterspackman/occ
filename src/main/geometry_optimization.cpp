...
std::pair<Wavefunction, Mat3N> run_method_for_optimization(const Molecule &m, const occ::qm::AOBasis &basis,
...
optimization_step_driver(const OccInput &config, const Molecule &m) {
	...
	switch(method_kind) {
	    case MethodKind::HF: {
		if (guess_sk == U || conf_sk == U) return run_method_for_optimization<HartreeFock, U>(m, basis, config);
		else if(guess_sk == G || conf_sk == G) return run_method_for_optimization<HartreeFock, G>(m, basis, config);
		else return run_method_for_optimization<HartreeFock, R>(m, basis, config);
		break;
	    }
	    case MethodKind::DFT: {
...
Wavefunction optimization_calculation(const OccInput &config) {
	...
	for(int i = 0; i < el.size(); i++) {
	    traj.print("{} {:12.6f} {:12.6f} {:12.6f}\n", el[i].symbol(), positions(0, i), positions(1, i), positions(2, i));
	}
    ...
    Eigen::Map<Mat3N>(x.data(), 3, m.positions().cols()) = m.positions() * occ::units::ANGSTROM_TO_BOHR;
