...
public:
  ...
  CEEnergyComponents operator()(const Dimer &dimer) {
    ...
    Wavefunction B = (m_wavefunctions_b.size() > 0)
  ...
  Mat3N electric_field(const Dimer &dimer) {
    ...
    occ::Mat3N pos_A_bohr = mol_A.positions() * occ::units::ANGSTROM_TO_BOHR;
  ...
  inline const auto &partial_charges() {
    ...
    for (int i = 0; i < m_wavefunctions_a.size(); i++) {
      m_partial_charges[i] = occ::qm::chelpg_charges(m_wavefunctions_a[i]);
    }
    return m_partial_charges;
  }
...
int compute_coulomb_energies_radius(const std::vector<Dimer> &dimers,
  ...
  for (const auto &dimer : dimers) {
    ...
    charge_energies[current_dimer] =
        occ::interaction::coulomb_interaction_energy_asym_charges(dimer,
                                                                  asym_charges);
    current_dimer++;
  }
...
int compute_dimer_energies_radius(
  ...
  for (size_t i = 0; i < dimers.size(); i++) {
    ...
    if (dimer.nearest_distance() > radius || dimer_energies[i].is_computed)
      continue;
    dimers_to_compute++;
  }

  PairEnergyStore store{PairEnergyStore::Kind::Xyz,
                        fmt::format("{}_dimers", basename)};
  ...
  for (const auto &dimer : dimers) {
    ...
    if (store.load(current_dimer, dimer, dimer_energy)) {
      progress.update(
          computed_dimers, dimers_to_compute,
    ...
    occ::log::debug(
        ...
        asym_idx_a, SymmetryOperation(a.asymmetric_unit_symop()(0)).to_string(),
        asym_idx_b, SymmetryOperation(b.asymmetric_unit_symop()(0)).to_string(),
        shift_b[0], shift_b[1], shift_b[2], dimer.center_of_mass_distance());
  ...
  progress.clear();
  occ::log::info(
      "Finished calculating {} unique dimer interaction energies in "
      "{:%H:%M:%S}",
      computed_dimers,
      std::chrono::round<std::chrono::seconds>(progress.time_taken()));
  return computed_dimers;
}
  ...
  void setup(const Crystal &crystal, EnergyModel &energy_model) {
    ...
    for (int i = 0; i < asym_mols.size(); i++) {
      ...
      charge_self_energies[i] =
          occ::interaction::coulomb_self_energy_asym_charges(mol, asym_charges);
...
converged_lattice_energies(EnergyModel &energy_model, const Crystal &crystal,
  ...
  occ::log::info("Radius increment   {: 8.4f} angstrom", conv.radius_increment);
  ...
  do {
    ...
    if (conv.wolf_sum) {
      compute_coulomb_energies_radius(dimers, wolf.asym_charges, current_radius,
                                      charge_energies);
    }
    ...
    for (const auto &n : mol_neighbors) {
        ...
        if (e.is_computed) {
          ...
          if (conv.wolf_sum) {
            ...
            if (conv.crystal_field_polarization) {
              wolf.electric_field_values[mol_idx] +=
                  compute_point_charge_efield(dimer, wolf.asym_charges);
            }
          }
          if (conv.crystal_field_polarization) {
      ...
      if (conv.wolf_sum) {
        ecoul_self +=
            wolf.charge_self_energies[mol_idx] * units::AU_TO_KJ_PER_MOL;
    ...
    occ::log::debug("Total coulomb term: {:.3f}", 0.5 * total.coulomb_kjmol());
  ...
  for (int i = 0; i < converged_energies.size(); i++) {
    if (i > converged_dimers.unique_dimers.size()) {
      throw std::runtime_error("Invalid dimer index for unique dimers when "
                               "propagating dimer energies");
    }
...
bool PairEnergyStore::save(int id, const Dimer &d,
                           const CEEnergyComponents &e) {
  fs::path parent(name);
  if (!fs::exists(parent)) {
    fs::create_directories(parent);
  }
  return write_xyz_dimer((parent / fs::path(dimer_filename(id, d))).string(), d,
                         e);
}

bool PairEnergyStore::load(int id, const Dimer &d, CEEnergyComponents &e) {
  fs::path parent(name);
  return load_dimer_energy((parent / fs::path(dimer_filename(id, d))).string(),
                           e);
}
