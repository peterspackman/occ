        ...
        const std::string& solvent_name)
{
    ...
    for(const auto& wfn: wfns)
    {
        ...
        props.e_ele = (props.dg_ele / coul_areas.array().sum()) * coul_areas.array();
        props.e_conc = (props.dg_conc / cds_areas.array().sum()) * cds_areas.array();
...
std::pair<occ::IVec, occ::Mat3N> environment(const std::vector<Dimer> &neighbors)
{
    ...
    for(const auto &n: neighbors)
    {
        ...
        positions.block(0, current_idx, 3, N) = n.b().positions() / BOHR_TO_ANGSTROM;
        current_idx += N;
        i++;
    }
        ...
        const std::string& solvent)
{
    ...
    for(size_t i = 0; i < neighbor_idx_coul.rows(); i++)
    {
        ...
        energy_contribution[mol_idx(idx)].coul_ab += surface.e_coulomb(i) + surface.e_ele(i);
    ...
    for(size_t i = 0; i < neighbor_idx_cds.rows(); i++)
    {
        ...
        Eigen::Index idx = 0;
        double r = (neigh_pos.colwise() - x).colwise().squaredNorm().minCoeff(&idx);
        energy_contribution[mol_idx(idx)].cds_ab += surface.e_cds(i) + surface.e_conc(i);
        ...
        const std::vector<CEModelInteraction::EnergyComponents> dimer_energies)
{
    ...
    for(size_t k1 = 0; k1 < solv.size(); k1++)
    {
        ...
        for(size_t k2 = 0; k2 < solv.size(); k2++)
        {
            ...
            double dp = v.dot(v2);
            if(dp <= 0.0) continue;
            crystal_contributions[k2] += (dp / total_dp) * dimer_energies[unique_dimer_idx].total_kjmol();
        }
    }
    double total_reassigned{0.0};
    for(size_t k1 = 0; k1 < solv.size(); k1++)
    {
        if(crystal_contributions[k1] == 0.0) continue;
...
int main(int argc, char **argv) {
    ...
    try {
        ...
        const std::string row_fmt_string = "{:>7.2f} {:>7.2f} {:>20s} {: 7.2f} {: 7.2f} {: 7.2f} {: 7.2f} {: 7.2f} {: 7.2f} {: 7.2f} {: 7.2f} {: 7.2f}\n";
        ...
        for(size_t i = 0; i < mol_neighbors.size(); i++)
        {
            ...
            fmt::print("{:>7s} {:>7s} {:>20s} {:>7s} {:>7s} {:>7s} {:>7s} {:>7s} {:>7s} {:>7s} {:>7s} {:>7s}\n",
                       "Rn", "Rc", "Symop", "E_coul", "E_rep", "E_pol", "E_disp", "E_tot", "E_scoul", "E_scds", "E_nn", "E_int");
            fmt::print("============================================================================================================\n");

            size_t j = 0;
            CEModelInteraction::EnergyComponents total; 

            auto neigh = fmt::output_file(fmt::format("{}_{}_neighbors.xyz", basename, i), fmt::file::WRONLY | O_TRUNC | fmt::file::CREATE);
            ...
            for(const auto& dimer: n)
            {
                ...
                fmt::print("{}: ({} {} {}) {}: ({} {} {})\n",
                           idx_a, shift_a[0], shift_a[1], shift_a[2], 
                           idx_b, shift_b[0], shift_b[1], shift_b[2]); 
            ...
            double dG_solubility = surfaces[i].esolv * occ::units::AU_TO_KJ_PER_MOL - 0.5 * total.total + Gr + Gt;
