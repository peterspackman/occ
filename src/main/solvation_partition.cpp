...
compute_solvation_energy_breakdown_nearest_atom(
  ...
  for (size_t i = 0; i < neighbor_idx_coul.rows(); i++) {
    ...
    cfile.print("{:12.5f} {:12.5f} {:12.5f} {:12.5f} {:5d}\n", angstroms(x(0)),
                angstroms(x(1)), angstroms(x(2)), surface.e_coulomb(i), m_idx);
  }
...
calculate_solvated_surfaces(const std::string &basename,
  ...
  for (const auto &wfn : wfns) {
    fs::path props_path(
        fmt::format("{}_{}_{}_surface.json", basename, index, solvent_name));
    fs::path json_wfn_path(
        fmt::format("{}_{}_{}.owf.json", basename, index, solvent_name));
    if (fs::exists(props_path)) {
      occ::log::info("Loading surface properties from {}", props_path.string());
      {
      ...
      if (fs::exists(json_wfn_path)) {
        ...
        occ::qm::AOBasis basis = occ::qm::AOBasis::load(wfn.atoms, basis_name);
        ...
        occ::log::info("Writing solvated wavefunction file to {}",
                       json_wfn_path.string());
      ...
      auto elec = proc_solv.surface_electronic_energy_elements(scf.molecular_orbitals());
