...
void register_crystal_bindings() {
    ...
    class_<HKL>("HKL")
        ...
        .property("k", &HKL::k)
        .property("l", &HKL::l)
        .function("d", optional_override([](const HKL &hkl, const UnitCell &cell) {
        ...
        .class_function("floor", optional_override([](const val &vecArray) {
            Vec3 v(vecArray[0].as<double>(), vecArray[1].as<double>(), vecArray[2].as<double>());
            return HKL::floor(v);
        }))
    ...
    class_<UnitCell>("UnitCell")
        ...
        .function("toCartesian", optional_override([](const UnitCell &cell, const val &fracArray) {
        ...
        .function("toFractional", optional_override([](const UnitCell &cell, const val &cartArray) {
    ...
    class_<SymmetryOperation>("SymmetryOperation")
        ...
        .function("getRotation", optional_override([](const SymmetryOperation &symop) {
        ...
        .function("getTranslation", optional_override([](const SymmetryOperation &symop) {
        ...
        .function("apply", optional_override([](const SymmetryOperation &symop, const val &coordsArray) {
    ...
    class_<SpaceGroup>("SpaceGroup")
        ...
        .function("getSymmetryOperations", optional_override([](const SpaceGroup &sg) {
        ...
        .function("numSymmetryOperations", optional_override([](const SpaceGroup &sg) {
    ...
    class_<AsymmetricUnit>("AsymmetricUnit")
        ...
        .function("generateDefaultLabels", &AsymmetricUnit::generate_default_labels)
        .function("getPositions", optional_override([](const AsymmetricUnit &asym) {
        ...
        .function("getAtomicNumbers", optional_override([](const AsymmetricUnit &asym) {
        ...
        .function("setPositions", optional_override([](AsymmetricUnit &asym, const val &posArray) {
        ...
        .function("setAtomicNumbers", optional_override([](AsymmetricUnit &asym, const val &numsArray) {
        ...
        .function("setLabels", optional_override([](AsymmetricUnit &asym, const val &labelsArray) {
    ...
    class_<Crystal>("Crystal")
        ...
        .function("asymmetricUnit", optional_override([](const Crystal &crystal) -> const AsymmetricUnit& {
            return crystal.asymmetric_unit();
        }), allow_raw_pointers())
        .function("spaceGroup", optional_override([](const Crystal &crystal) -> const SpaceGroup& {
            return crystal.space_group();
        }), allow_raw_pointers())
        .function("unitCell", optional_override([](const Crystal &crystal) -> const UnitCell& {
            return crystal.unit_cell();
        }), allow_raw_pointers())
        .function("numSites", &Crystal::num_sites)
        .function("volume", &Crystal::volume)
        .function("chemicalFormula", optional_override([](const Crystal &crystal) {
            return crystal.asymmetric_unit().chemical_formula();
        }))
        .function("toFractional", optional_override([](const Crystal &crystal, const val &cartArray) {
            Mat3N cart_coords(3, cartArray["length"].as<int>() / 3);
            for (int i = 0; i < cartArray["length"].as<int>() / 3; ++i) {
                cart_coords(0, i) = cartArray[i * 3 + 0].as<double>();
                cart_coords(1, i) = cartArray[i * 3 + 1].as<double>();
        ...
        .function("toCartesian", optional_override([](const Crystal &crystal, const val &fracArray) {
            Mat3N frac_coords(3, fracArray["length"].as<int>() / 3);
            for (int i = 0; i < fracArray["length"].as<int>() / 3; ++i) {
                frac_coords(0, i) = fracArray[i * 3 + 0].as<double>();
                frac_coords(1, i) = fracArray[i * 3 + 1].as<double>();
        ...
        .class_function("fromCifFile", optional_override([](const std::string &filename) {
            occ::io::CifParser parser;
            return parser.parse_crystal_from_file(filename).value();
        }))
        .class_function("fromCifString", optional_override([](const std::string &cifContent) {
            occ::io::CifParser parser;
            return parser.parse_crystal_from_string(cifContent).value();
        }));

    // Utility functions
    function("parseSpaceGroupNumber", optional_override([](const std::string &symbol) {
