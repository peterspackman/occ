...
void register_cube_bindings() {
    ...
    class_<io::Cube>("Cube")
        ...
        .function("setOrigin", optional_override([](io::Cube &cube, double x, double y, double z) {
        ...
        .function("setBasis", optional_override([](io::Cube &cube, const val &basisArray) {
            // Expect a 3x3 matrix as a flat array of 9 elements
            if (basisArray["length"].as<int>() != 9) {
                throw std::runtime_error("Basis matrix must be 3x3 (9 elements)");
            }
            for (int i = 0; i < 3; ++i) {
                for (int j = 0; j < 3; ++j) {
        ...
        .function("setSteps", optional_override([](io::Cube &cube, int nx, int ny, int nz) {
        ...
        .function("addAtom", optional_override([](io::Cube &cube, int atomicNumber, double x, double y, double z) {
        ...
        .function("getData", optional_override([](const io::Cube &cube) {
            size_t totalSize = cube.steps(0) * cube.steps(1) * cube.steps(2);
        ...
        .function("setData", optional_override([](io::Cube &cube, const val &dataArray) {
            size_t totalSize = cube.steps(0) * cube.steps(1) * cube.steps(2);
            if (dataArray["length"].as<size_t>() != totalSize) {
                throw std::runtime_error("Data array size must match grid dimensions");
            }
        ...
        .function("fillFromElectronDensity", optional_override([](io::Cube &cube, const qm::Wavefunction &wfn) {
        ...
        .function("save", optional_override([](io::Cube &cube, const std::string &filename) {
            cube.save(filename);
        }))
    ...
    function("loadCubeFromString", optional_override([](const std::string &cubeData) {
