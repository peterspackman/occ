...
double XTBCalculator::single_point_energy() {
  ...
  std::vector<std::string> command_line{m_xtb_executable_path,
                                        xtbinput_filename, "--json", "--grad"};
  if (!m_solvent.empty()) {
  ...
  auto process = subprocess::run(
      command_line, RunBuilder().cerr(PipeOption::pipe).cout(PipeOption::pipe));
...
void XTBCalculator::write_input_file(const std::string &dest) {
  ...
  for (int i = 0; i < num_atoms(); i++) {
    fmt::print(of, "{:20.12f} {:20.12f} {:20.12f} {}\n", m_positions_bohr(0, i),
               m_positions_bohr(1, i), m_positions_bohr(2, i),
