...
double w3a(double l1, double l2, double l3, double m1, double /*m2*/, double /*m3*/) {
...
OCC_ALWAYS_INLINE double alpha_term_l1_zero(double l1, double l2, double l3, double m1, double m2, double m3) {
    return -(m3 - m2 + 2.0 * w3b(l1, l2, l3, m1, m2, m3)) / w3a(1.0, l2, l3, m1, m2, m3);
}

OCC_ALWAYS_INLINE double alpha_term(double l1, double l2, double l3, double m1, double m2, double m3) {
    return -w3b(l1, l2, l3, m1, m2, m3) / (l1 * w3a(l1 + 1.0, l2, l3, m1, m2, m3));
}

OCC_ALWAYS_INLINE double alpha_term_backward(double l1, double l2, double l3, double m1, double m2, double m3) {
    return -w3b(l1, l2, l3, m1, m2, m3) / ((l1 + 1.0) * w3a(l1, l2, l3, m1, m2, m3));
}

OCC_ALWAYS_INLINE double beta_term(double l1, double l2, double l3, double m1, double m2, double m3) {
    return -(l1 + 1.0) * w3a(l1, l2, l3, m1, m2, m3) / (l1 * w3a(l1 + 1.0, l2, l3, m1, m2, m3));
}

OCC_ALWAYS_INLINE double beta_term_backward(double l1, double l2, double l3, double m1, double m2, double m3) {
    return -l1 * w3a(l1 + 1.0, l2, l3, m1, m2, m3) / ((l1 + 1.0) * w3a(l1, l2, l3, m1, m2, m3));
}
...
Vec wigner3j(double l2, double l3, double m1, double m2, double m3) {
    ...
    if(!(std::fabs(m1 + m2 + m3) < eps &&
	 std::fabs(m2) <= l2 + eps &&
	 std::fabs(m3) <= l3 + eps)) return Vec::Zero(1);
	...
	if (l1min_zero) an = impl::alpha_term_l1_zero(l1, l2, l3, m1, m2, m3);
	else an = impl::alpha_term(l1min, l2, l3, m1, m2, m3);
	...
	if (size > 2) {
		...
		double lambda = (
		    l1_mid_plus_one * result(j + 2) + l1_mid * result(j + 1) + l1_mid_minus_one * result(j)) /
